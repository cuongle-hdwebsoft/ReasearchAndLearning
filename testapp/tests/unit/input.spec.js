...
describe('Test Custom Input', function() {
  ...
  it('should change when type', async function() {
    const handleChangeInput = sandbox.spy(CustomInput.methods, 'handleChangeInput')
    ...
    expect(handleChangeInput.getCall(0).args[0].target.value).to.be.equal(words)
  ...
  it('should be error when string length is greater than 5', async function() {
    ...
    expect(error.exists()).to.be.true
    expect(error.element.innerHTML).to.be.equal('value.length is greater than 5')
  })
})