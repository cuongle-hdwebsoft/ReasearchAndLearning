...
describe('App', function() {
  ...
  it('should render lists DOM', async function() {
    ...
    for(let i = 0 ; i < domItems.length; i++) {
      ...
      expect(domItem.element.innerHTML).to.be.equal(`${ todo.id } - ${ todo.title } - ${ todo.description }`)
    }
  })

  it('should stub mixin methods', function() {
    const word = faker.word.adjective()
    const generatePk = sandbox.stub(TestMixin.methods, 'generatePk').returns(word)
  ...
  it('test', async function() {
    ...
    expect(payload.error.message).to.be.equal('Request failed with status code 500')
  })
})